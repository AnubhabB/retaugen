...
impl PdfProc {
    ...
    pub fn extract(&self) -> Result<Vec<Vec<(String, FileKind)>>> {
        // for each `.pdf` file we are going to convert the pages to images
        let file_encoded = self.pdfs
            .iter()
            .filter_map(|file| {
                let pdf = self.pdfium.load_pdf_from_file(&file, None).ok()?;
                
                self.process_pages(file, pdf)
            }).collect::<Vec<_>>();

        Ok(file_encoded)
    }

    pub fn process_pages(&self, file: &PathBuf, doc: PdfDocument<'_>) -> Option<Vec<(String, FileKind)>> {
        Some(
        doc.pages()
            .iter()
            .enumerate()
...
impl Extractor {
    ...
    pub async fn extract(&self, send: Sender<ExtractorEvt>) -> Result<()> {
        let mut results = vec![];
        if let Some(pdf) = &self.pdfproc {
            results = [&results, &pdf.extract()?[..]].concat();
        }

        if !self.txts.is_empty() {
            results = [&results, &self.extract_txt()?[..]].concat();
        }
        
        send.send(ExtractorEvt::Data(Ok(results))).await.map_err(|e| anyhow!(e))
    }

    // Reads text from each `.pdf` file and returns the text per page for the file
    pub fn extract_txt(&self) -> Result<Vec<Vec<(String, FileKind)>>> {
        let d = self.txts
            .iter()
            .filter_map(|f| {
